// src/components/TeacherImageUpload.tsx  (or wherever you keep your components)

import React, { useState } from "react";
import { Upload, Image as ImageIcon, X, CheckCircle, AlertCircle } from "lucide-react";

const TeacherImageUpload: React.FC = () => {
      ...
      case "grade12":
        if (stream === "natural") {
          return [
            ...
            `${category === "grade11" ? "Grade 11" : "Grade 12"} Mathematics`,
  ...
  const handleFile = (file: File) => {
    ...
    if (ext && allowed.includes(ext)) {
      ...
      setError("Invalid file type. Only JPG, JPEG, PNG, GIF, WEBP allowed.");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Stricter validation before sending
    if (!selectedFile) {
    ...
    if (!trimmedTitle) {
      setError("Please enter a descriptive image title.");
      return;
    }
    if (["grade11", "grade12", "entrance", "remedial"].includes(category) && !stream) {
      setError("Please select Natural Science or Social Science stream.");
      return;
    }
    ...
    if (stream?.trim()) {
      formData.append("stream", stream.trim());
    }
    ...
    console.log("Sending FormData to /image-upload/:");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-pink-50 to-purple-100 p-6">
      <div className="max-w-4xl mx-auto">
        {/* Quick visual indicator – helps during development */}
        <div className="bg-purple-100 p-4 mb-6 rounded-xl text-center text-purple-800 font-bold">
          IMAGE UPLOAD FORM – JPG / PNG / WEBP / GIF only
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-purple-200">
          <div className="bg-gradient-to-r from-purple-600 to-pink-600 p-6 text-white">
            <h4 className="text-2xl font-bold flex items-center gap-3">
              <ImageIcon size={28} />
              Upload Teaching Image / Diagram / Photo
            </h4>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-8">
            ...
            {/* Stream – only shown when needed */}
            {(category === "grade11" || category === "grade12" || category === "entrance" || category === "remedial") && (
              <div>
                <label className="block text-lg font-semibold text-purple-800 mb-3">
                  Stream <span className="text-red-500 text-sm">*</span>
                </label>
            ...
            <div>
              <label className="block text-lg font-semibold text-purple-800 mb-3">
                Image Title <span className="text-red-500 text-sm">*</span>
              </label>
              <input
                type="text"
                value={imageTitle}
              ...
              <p className="text-sm text-purple-600 mt-1">
                Give a clear, descriptive title — it helps students find and understand the image.
              </p>
            </div>
            ...
            <div>
              <label className="block text-lg font-semibold text-purple-800 mb-3">
                Upload Image <span className="text-red-500 text-sm">*</span> (JPG, JPEG, PNG, GIF, WEBP)
              </label>
              <div
              ...
              >
                ...
                <p className="text-xl font-semibold text-purple-800">Drag & drop your image here</p>
            ...
            {uploadSuccess && (
              <div className="p-4 bg-green-50 border-2 border-green-300 rounded-xl flex items-center gap-3">
                ...
                <p className="text-green-800 font-bold text-lg">
                  Image uploaded successfully! Waiting for director approval.
                </p>
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={uploading || !selectedFile || !subject.trim() || !imageTitle.trim() || !category}
              className={`w-full py-5 text-xl font-bold text-white rounded-xl transition-all transform hover:scale-105 shadow-lg flex items-center justify-center gap-3 ${
                uploading || !selectedFile || !subject.trim() || !imageTitle.trim() || !category
        ...
        <p className="text-center text-purple-700 mt-8 text-sm italic">
          All uploaded images will be reviewed and made available to students across Ethiopia based on the New Curriculum.
        </p>
